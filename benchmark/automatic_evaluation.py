...
1. The original experiment CSV generated by the preprocessing/generation pipeline.
2. A directory of `HAI-UIST-DATA/*.json` session exports.

For session-JSON inputs, this script locally stages scenario sentences and
counterfactual plans into the five Gross Process Model phases so the automatic
metrics can be computed without the original staged CSV.
"""
